...
pub(crate) fn smoc_from_json(data: &str) -> Result<InternalMoc, String> {
    let cells = from_json_aladin::<u64, Hpx<u64>>(data).map_err(|e| e.to_string())?;
    let moc = cells.into_cell_moc_iter().ranges().into_range_moc();
    Ok(InternalMoc::Space(moc))
}

pub(crate) fn tmoc_from_json(data: &str) -> Result<InternalMoc, String> {
    let cells = from_json_aladin::<u64, Time<u64>>(data).map_err(|e| e.to_string())?;
    let moc = cells.into_cell_moc_iter().ranges().into_range_moc();
    Ok(InternalMoc::Time(moc))
}

pub(crate) fn stmoc_from_json(data: &str) -> Result<InternalMoc, String> {
